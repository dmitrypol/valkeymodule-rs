...
impl ValkeyString {
    pub fn test<T: Into<Vec<u8>>>(data: T) -> ValkeyString {
        ...
        let inner = data as *mut raw::RedisModuleString;
        ValkeyString::from_redis_module_string(std::ptr::null_mut(), inner)
    }
}

pub unsafe extern "C" fn test_string_ptr_len(
...
pub unsafe extern "C" fn test_free_string(
    _ctx: *mut raw::RedisModuleCtx,
    _string: *mut raw::RedisModuleString,
) {
}

pub unsafe extern "C" fn test_retain_string(
    _ctx: *mut raw::RedisModuleCtx,
    _string: *mut raw::RedisModuleString,
) {
}

pub unsafe extern "C" fn test_string_to_longlong(
    string: *const raw::RedisModuleString,
    value: *mut i64,
) -> libc::c_int {
    let data = unsafe { &*(string as *const Vec<u8>) };
