...
// Run during process initialization so tests can use `ValkeyString` before
// they explicitly invoke any setup helpers.
#[cfg(any(test, feature = "test-mocks"))]
#[ctor::ctor]
fn ctor_setup_test_shims() {
    setup_test_shims();
}
#[cfg(any(test, feature = "test-mocks"))]
pub(crate) fn setup_test_shims() {
...
mod tests {
    ...
    fn test_string_inner_returns_original_allocation() {
        let s = ValkeyString::create(None, "hello");
    ...
    fn create_for_test_accepts_byte_buffers() {
        let s = ValkeyString::create(None, vec![0x66, 0x6f, 0x6f]);
    ...
    fn invalid_utf8_is_preserved_in_slice_accessors() {
        let s = ValkeyString::create(None, vec![0xff, b'o', b'o']);
    ...
    fn empty_string_is_empty() {
        let s = ValkeyString::create(None, "");
        assert_eq!(s.len(), 0);
        assert!(s.is_empty());
    }
    ...
    fn ordering_and_equality() {
        let a = ValkeyString::create(None, "aaa");
        let b = ValkeyString::create(None, "bbb");
        let a2 = ValkeyString::create(None, "aaa");
        assert!(a < b);
        assert!(b > a);
        assert_eq!(a, a2);
    }
    ...
    fn clone_creates_independent_string() {
        let mut s = ValkeyString::create(None, "41");
        let cloned = s.clone();
    ...
    fn append_returns_ok_and_updates_bytes() {
        let mut s = ValkeyString::create(None, "foo");
    ...
    fn integer_and_float_parsing_work() {
        let integer = ValkeyString::create(None, "41");
        let float = ValkeyString::create(None, "41.5");
    ...
    fn parsing_errors_match_expected_messages() {
        let invalid_int = ValkeyString::create(None, "abc");
        let negative = ValkeyString::create(None, "-1");
        let invalid_float = ValkeyString::create(None, "abc");
    ...
    fn borrow_returns_placeholder_for_invalid_utf8() {
        let s = ValkeyString::create(None, vec![0xff, b'o']);
